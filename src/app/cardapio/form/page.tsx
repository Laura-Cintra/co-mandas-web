            ...
            <main className="flex justify-center">
                <div className="p-6 w-[100%]"> 
                    ...
                    <form action={formAction} className="space-y-4 mt-6">
                        <div>
                            <Input name="name" placeholder="Nome do prato" aria-invalid={!!state?.errors.name} className="w-full p-3 border border-gray-300 rounded-lg bg-white text-gray-900" />
                            <span className="text-sm text-red-500">{state?.errors.name}</span>
                        </div>

                        <div>
                            <Input type="number" name="price" step="0.01" min="0" placeholder="Preço (R$)" aria-invalid={!!state?.errors.price} className="w-full p-3 border border-gray-300 rounded-lg bg-white text-gray-900" />
